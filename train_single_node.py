...
def train_model(dataset_split, training_config: TrainingConfig):
    ...
    params['verbose'] = 1
    model = lgb.train(params, train_set=train_data, valid_sets=[dev_data])
...
def main():
    ...
    for split in make_splits(X,y, rng):
        train_model(split, training_config)
